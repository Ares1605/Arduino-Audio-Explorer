...
NOTE_G4,8, NOTE_C4,8, NOTE_DS4,16, NOTE_F4,16, NOTE_G4,8, NOTE_C4,8, NOTE_DS4,16, NOTE_F4,16,
  ...
  NOTE_C6,8, NOTE_G5,16, NOTE_GS5,16, NOTE_AS5,16, NOTE_C6,8, NOTE_G5,8, NOTE_GS5,16, NOTE_AS5,
"""

# Function to replace notes with frequencies
def convert_notes(sequence):
    result = []
    elements = sequence.split(",")  # Split by commas
    for element in elements:
        note = element.strip()  # Remove extra spaces
        if note in NOTE_FREQUENCIES:
            result.append(str(NOTE_FREQUENCIES[note]))  # Replace with frequency
        else:
            result.append(note)  # Keep durations or unknown parts unchanged
    return ", ".join(result)
...
output_file = "song4.txt"
with open(output_file, "w") as file:
    file.write(converted_sequence)

print(f"Converted sequence written to {output_file}")
